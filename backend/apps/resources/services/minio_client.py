...
def build_video_object_key(filename: str, *, tenant: 'Tenant') -> str:
    ...
    return f'{tenant_object_prefix(tenant)}videos/{today:%Y/%m/%d}/{uuid.uuid4().hex}{ext}'
...
def build_public_object_url(object_key: str, settings: MinioSettings | None = None) -> str:
    if not object_key:
        return ''
