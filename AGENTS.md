...
- 图标库统一使用 **lucide-react**，不再使用 `@ant-design/icons`。
- lucide-react 名称与 JS 保留字或 React 内置类型冲突时（如 `Key`），用 `as` 别名：`import { Key as KeyIcon } from 'lucide-react'`。
- 图标大小通过 `size` 属性控制，不依赖 Tailwind `text-*` 类缩放 SVG。
- 每个文件只从 `lucide-react` 统一导入一次，不混用多套图标库。
